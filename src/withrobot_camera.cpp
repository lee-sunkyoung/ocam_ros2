...
bool Camera::get_capability() {
  ...
  DBG_PRINTF("V4L2_CAP_STREAMING : %s",
             (v4l2_s.capability.capabilities & V4L2_CAP_STREAMING) ? "True"
                                                                   : "False");
...
bool Camera::set_buffer() {
  ...
  for (unsigned int i = 0; i < buffer_count; i++) {
    ...
    buffers[i].buffer = (unsigned char *)mmap(
        NULL, v4l2_s.buffer.length, PROT_READ | PROT_WRITE, MAP_SHARED, fd,
        v4l2_s.buffer.m.offset);
...
bool Camera::enumerate_frame_sizes(const unsigned int pixelformat,
  ...
  while (xioctl(VIDIOC_ENUM_FRAMESIZES, &v4l2_s.frmsizeenum) != -1) {
    ...
    case V4L2_FRMSIZE_TYPE_STEPWISE:
      DBG_PRINTF("Stepwise :: Width step(max/min): %d (%d / %d), Height "
                 "step(max / min): %d (%d / %d)",
